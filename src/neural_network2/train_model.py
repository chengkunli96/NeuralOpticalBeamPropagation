...
# from utils.train_phase import train_net
from model.net.model import Net as Net
from utils.eval import eval_net
from utils.dataset import BasicDataset
from utils.showfigure import showFigure
...
def train_net(net,
    ...
    angle_input = batch['angle']
    angle_input = torch.unsqueeze(angle_input, 1)
    input_data = torch.cat((amplitude_input, phase_input), 1)
    input_data = input_data.to(device=device, dtype=torch.float32)
    angle = angle_input.to(device=device, dtype=torch.float32)
    writer.add_graph(net, [input_data, angle])
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                angle_input = batch['angle']
                angle_input = torch.unsqueeze(angle_input, 1)  # shape: BC
                ...
                if global_step % (n_train // (10 * batch_size)) == 0:  # print for every 10 batches
                    ...
                    # tensorboard angle
                    tb_angle = angle.data.cpu().numpy()
                    tb_angle_img = np.zeros(shape=(tb_angle.shape[0], 1, 256, 256))
                    for i in range(tb_angle.shape[0]):
                        tb_angle_img[i, 0, :, :] = showFigure(tb_angle[i])
                    tb_angle_img = torch.from_numpy(tb_angle_img)
                    
                    writer.add_images('input/angle',tb_angle_img, global_step)
...
dataset0_pth = opj(CURR_FILE_PATH, '../../data/processed/lfw_nn2')

# in terminal, you can run
# python train_model.py --epochs 20  --batch-size 4  --learning-rate 0.001 --validation 30 --device 0
...
if __name__ == '__main__':
    ...
    additiontxt = '''Addition:\n
        Model1: res-unet + dbpnet. 2 channels in with angle info and 2 channels out.\n
        Input1: (angle) -> (FC16 + FC256) -> (conv16->conv64) = 64 maps (4 times 4) and added in the bottle neck of the unet\n
        Input2: (2 Channels amplitude and phase images)\n
        Training: LFW wiht a random tilted angle whose degree is in range [0, 45].\n
        Loss: MSE\n
    '''
    # dataset set
    ...
    # ===== network setting =====
    # set neural network
    net = Net(in_channels1=2, in_channels2=1, out_channels=2)
